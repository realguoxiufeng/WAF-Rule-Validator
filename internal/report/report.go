...
// ExportFullReport saves full report on disk in different formats: HTML, PDF, JSON, DOCX.
func ExportFullReport(
	...
	for _, format := range formats {
		...
		case DocxFormat:
			reportFileName = reportFile + ".docx"
			err = printFullReportToDocx(s, reportFileName, reportTime, wafName, url, openApiFile, args, ignoreUnresolved, includePayloads)
			if err != nil {
				return nil, err
			}
